...
# Models
import torch
from transformers import HubertForCTC, T5Tokenizer, T5ForConditionalGeneration, Wav2Vec2ForCTC, Wav2Vec2Processor, Wav2Vec2Tokenizer
...
def transcript_from_microphone(stt_tokenizer, stt_model, t5_tokenizer, t5_model, summarizer, dia_pipeline):
    """
    Record audio directly from the microphone using the st_audiorec component and process the recorded audio to
    transcribe, punctuate, and apply diarization if selected.
    
    Parameters:
    - stt_tokenizer: Speech to text model's tokenizer
    - stt_model: Speech to text model
    - t5_tokenizer: Auto punctuation model's tokenizer
    - t5_model: Auto punctuation model
    - summarizer: Summarizer model
    - dia_pipeline: Diarization Model (Differentiate speakers)
    """
    
    # Use the custom Streamlit component to record audio
    wav_audio_data = st_audiorec()
    
    # If audio data is provided by the component
    if wav_audio_data is not None:
        # Display the recorded audio to the user
        st.audio(wav_audio_data, format='audio/wav')

        # Save the audio data to a .wav file
        filename = 'recorded_audio.wav'  # Define the filename variable
        with open(filename, 'wb') as f:
            f.write(wav_audio_data)  # Write the binary audio data to a file
        update_session_state=["page_index", 0,]
        # Here you can call your existing transcription process using the filename
        transcription(stt_tokenizer, stt_model, t5_tokenizer, t5_model, summarizer, dia_pipeline, filename)
    else:
        st.write("No audio recorded. Please record some audio.")
...
def extract_audio_from_yt_video(url):
    """
    Extracts audio from a YouTube URL using Pytube.
    :param url: link of a YouTube video
    :return: name of the saved audio file or None if an error occurs
    """
    try:
        yt = YouTube(url)
        audio_streams = yt.streams.filter(only_audio=True).first()
        if audio_streams:
            filename = f"yt_download_{yt.video_id}.mp3"
...
if __name__ == '__main__':
    ...
    if st.session_state['page_index'] == 0:
        # Home page
        choice = st.radio("Features", ["By a video URL", "By uploading a file","By Recording"])
        ...
        if choice == "By a video URL":
            ...
            transcript_from_microphone(stt_tokenizer, stt_model, t5_tokenizer, t5_model, summarizer, dia_pipeline)
